...
const HeadingWithAnchor = Heading.extend<HeadingWithAnchorOptions>({
  addOptions() {
    ...
    return {
      ...
      levels: parentOptions?.levels ?? [1, 2, 3, 4, 5, 6], // Default levels if not provided by parent
...
export function scrollToCurrentHeadingAnchor(editor: Editor) {
  ...
  if (elementForHash && editor.options.element && editor.options.element.contains(elementForHash)) {
