...
function getProfileImageSrc(photo: string | null | undefined) {
  ...
  if (!photo) return "/avatars/neutral.jpg";
  if (photo.startsWith("http")) return photo;
  if (photo.startsWith("media") || photo.startsWith("/media")) {
    return `${BACKEND_BASE_URL.replace(/\/$/, "")}/${photo.replace(/^\/?/, "")}`;
  }
  return "/" + photo;
}
...
function LoginV1() {
  ...
  const onLoginSubmit = async (data: z.infer<typeof LoginSchema>) => {
    try {
      ...
      localStorage.setItem("admin_photo", getProfileImageSrc(result.admin.photo));
